...
export default {
    ...
    mutations: {
        ...
        addHistoryForum(state, {fid, name, recommend}) {
            ...
            state.forums.splice(0, 0, {fid: parseInt(fid), name: n, recommend})
            //    保存版面浏览记录
            putCache("ForumHistory", state.forums)
        },
        addHistoryThread(state, {tid, name}) {
            // slf4j(`添加主题浏览记录 tid:${tid} name:${name}`)
            state.threads = state.threads.filter(i => parseInt(i.tid) !== parseInt(tid))
        ...
        addHistorySet(state, {stid, name, forumName}) {
            ...
            state.sets.splice(0, 0, {stid: parseInt(stid), name: n})
