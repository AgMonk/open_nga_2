// 官方表情前缀    https://img4.nga.178.com/ngabbs/post/smile/
// 论坛附件前缀    https://img.nga.178.com/attachments
...
export const mapEmoteToArray = ({name, namespace, urlType, data}) => {
    return Object.keys(data).map(k => {
        const u = data[k]
        const uuid = `${name}:${namespace}:${k}`
        let code;
        let url;
        if (urlType === '官方') {
            code = `[s:${namespace}:${k}]`
